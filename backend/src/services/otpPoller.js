...
async function tick() {
  const { rows } = await query(`
    SELECT q.order_id, q.provider, q.provider_order_id, q.poll_count,
           o.user_id, o.user_price, o.service, o.phone_number,
  ...
  await Promise.allSettled(rows.map(async (row) => {
    try {
      ...
      if (status === 'received' && sms) {
        ...
        // Email the OTP to the user — fire and forget
        sendOTPEmail({
          to:      row.user_email,
          otp,
          service: row.service,
          number:  row.phone_number,
        }).catch((err) => logger.warn('OTP email failed', { orderId: row.order_id, error: err.message }));

        logger.info('OTP received', { orderId: row.order_id, service: row.service });
        return;
      }
      ...
      await query(
        `UPDATE otp_poll_queue
         SET poll_count = $1, next_poll_at = NOW() + interval '5 seconds'
         WHERE order_id = $2`,
...
async function refundExpired(orderId, userId, amount) {
  ...
  try {
    ...
    await client.query(
      `UPDATE orders SET status = 'expired', updated_at = NOW() WHERE id = $1`, [orderId]
    );
    await client.query(
...
export function startPoller() {
  ...
  setInterval(async () => {
    try { await tick(); } catch (err) {
      logger.error('Poller tick error', { error: err.message });
    }
  }, INTERVAL_MS);
}
...
export async function recoverPending() {
  const { rows } = await query(`
    ...
    FROM orders o
    LEFT JOIN otp_poll_queue q ON q.order_id = o.id
